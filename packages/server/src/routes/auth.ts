...
auth.get("/login", (c) => {
  ...
  const proto = c.req.header("x-forwarded-proto") ?? (isSecure() ? "https" : "http")
  ...
  c.header("Set-Cookie", `pkce_verifier=${codeVerifier}; ${cookieFlags}`, { append: true })
...
auth.get("/callback", async (c) => {
  ...
  // Fetch user info
  const { data: ghUser, grantedScopes } = await githubFetch<GitHubUser>(tokenData.access_token, "/user")
  ...
  if (grantedScopes === null) {
    return c.json({ error: "Could not verify OAuth scopes — please try signing in again." }, 403)
  }
...
auth.post("/logout", async (c) => {
  ...
  c.header("Set-Cookie", `session=; HttpOnly; SameSite=Strict; Max-Age=0; Path=/`)
