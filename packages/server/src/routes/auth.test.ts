...
vi.mock("../middleware/auth.js", () => ({
  authMiddleware: vi.fn(async (c: import("hono").Context, next: () => Promise<void>) => {
    c.set("session", { sub: "42", login: "octocat", name: "The Octocat", avatarUrl: "https://avatars.githubusercontent.com/u/583231", sessionId: "s1" })
    await next()
  }),
}))
...
function mockTokenExchangeSuccess(accessToken = "gha_token") {
  mockUndiciRequest.mockResolvedValue({
    statusCode: 200,
    body: {
      json: () => Promise.resolve({ access_token: accessToken, token_type: "bearer", scope: "repo" }),
...
describe("GET /auth/callback", () => {
  ...
  it("returns 400 when state does not match cookie", async () => {
    const res = await makeCallbackRequest({ state: "real", stateCookie: "fake" })
    expect(res.status).toBe(400)
  })
  ...
  it("sends code_verifier in the token exchange body", async () => {
    ...
    const callBody = JSON.parse(mockUndiciRequest.mock.calls[0]![1]!.body as string)
    expect(callBody.code_verifier).toBe("my-pkce-verifier-value")
  })

  it("returns 504 when GitHub token exchange times out", async () => {
    mockUndiciRequest.mockRejectedValue(Object.assign(new Error("abort"), { name: "AbortError" }))
...
describe("POST /auth/logout", () => {
  it("clears the session cookie", async () => {
    mockVerifySession.mockResolvedValueOnce({
      sub: "42", sessionId: "s1", login: "octocat", name: "Octocat", avatarUrl: "https://avatars.githubusercontent.com/u/1"
    })
    const res = await makeApp().request("/auth/logout", { method: "POST", headers: { cookie: "session=some.jwt" } })
    const cookies = res.headers.getSetCookie()
  ...
  it("destroys the server-side session", async () => {
    mockVerifySession.mockResolvedValueOnce({
      sub: "42", sessionId: "sid-to-destroy", login: "octocat", name: "Octocat", avatarUrl: "https://avatars.githubusercontent.com/u/1"
    })
    await makeApp().request("/auth/logout", { method: "POST", headers: { cookie: "session=some.jwt" } })
  ...
  it("responds 200 when JWT verification fails (already expired)", async () => {
    ...
    const res = await makeApp().request("/auth/logout", { method: "POST", headers: { cookie: "session=bad.jwt" } })
    expect(res.status).toBe(200)
    expect(mockSessionDestroy).not.toHaveBeenCalled()
  })
})
