...
export async function githubFetch<T>(
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
    ...
    try {
      ...
      const rateLimitRemaining = parseIntHeader(response.headers["x-ratelimit-remaining"])
      const rateLimitReset = parseIntHeader(response.headers["x-ratelimit-reset"])
      ...
      if (status === 304) {
        ...
        return { data: null as unknown as T, rateLimitRemaining, rateLimitReset, status: 304, etag: etag ?? null, grantedScopes: null }
      }
      ...
      if (status === 302) {
        ...
        if (!location) throw new GitHubApiError(`302 redirect with no Location for ${path}`, 502)
      ...
      if (status >= 400) {
        ...
        const grantedScopes = parseStringHeader(response.headers["x-oauth-scopes"])
      ...
      // 204 No Content
      if (status === 204 || !text) {
        return { data: null as unknown as T, rateLimitRemaining, rateLimitReset, status, etag: null, grantedScopes: null }
      }
